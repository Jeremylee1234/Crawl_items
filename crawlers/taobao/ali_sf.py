		...
		# try:
			if driver == None:
				...
				for data in datas:
					...
					item.status = utils.get_status(data.find_elements_by_xpath('./div[contains(@class,"flag-section")]/div'))
...
class Parse_detail(object):
	...
	def parse_detail(self, item:Item):
		if self.err_times <= 1:
			item.repeat = '649351497323'
			item.url = 'https://sf-item.taobao.com/sf_item/649351497323.htm?track_id=e0c9af69-c401-4f7d-a1dd-d1b37e7f3568'
			item.people_bid = 100
			...
			if response.status_code == 200:
				...
				try:
					...
					attaches = json.loads(utils.escape_jsonp(response.text, 'list'))
