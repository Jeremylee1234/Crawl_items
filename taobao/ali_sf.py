...
class Get_urls(object):
    ...
    def access_page(self, province, category, page, driver=None):
        ...
        """
        if not driver:
            driver = Driver('https://sf.taobao.com/').create_driver()
        if self.err_times <= 5:
            if driver == None:
                # 判断在多次尝试创建浏览器窗口后失败
                self.err_times = 0
            ...
            try:
                ...
                # 其他错误,处理错误
                logger.info(f'尝试访问ali-sf二级页面失败:{url},重试')
                self.err_times += 1
                return None
    ...
    def parse_urls(self, province, category, driver):
        try:
            if driver == None:
                ...
                for data in datas:
                    try:
                        item = Item(str(re.match(pattern_sf, item['url']).group(1)), 'ali')
                        ...
                        item.category_id = category[3]
                        item.province = province[0]
                        item.province_id = province[1]
                        if not item.repeat in self.repeat_set:
                            self.repeat_set.append(item.repeat)
                            results.append(item)
                    except Exception as e:
                        logger.error(f'解析ali-sf一级页面某项时出错:{e}')
                        continue
                return results
        except Exception as e:
            logger.error(f'解析ali-sf一级页面整个页面时出错:{e}')
            return []
...
class Parse_detail(object):
	def __init__(self) -> None:
		...
		self.useful_cookies = db.get_cookies('ali-pc',True)
		...
		if len(self.useful_cookies) >= 1:
			...
			self.session = utils.get_session(self.cookie)
		else:
			update_all_cookies()
			self.useful_cookies = db.get_cookies('ali-pc',True)
			self.cookie_obj = random.choice(self.useful_cookies)
	...
	def parse_detail(self, item:Item):
		if self.err_times <= 1:
			response = self.session.get(url=item.url, headers=consts.sf_pc_detail_headers)
			if response.status_code == 200:
				try:
					...
					item.people_contact = utils.clean(utils.list_to_str(selector.xpath('//div[contains(@class,"contact-unit")]//text()')))
					...
					item.start_date = utils.formate_timestamp(utils.outFirst(selector.xpath('//li[@id="sf-countdown"]/@data-start')))
					item.end_date = utils.formate_timestamp(utils.outFirst(selector.xpath('//li[@id="sf-countdown"]/@data-end')))
					...
					for info in infos:
						...
						if not item.get('current_price') and '变卖预缴款' in info:
					...
					if isinstance(url1, str):
						if 'http' not in url1:
							url1 = 'https:' + url1
						response = self.session.get(url1, headers=consts.sf_pc_detail_headers)
					...
					if isinstance(url2, str):
						if 'http' not in url2:
							url2 = 'https:' + url2
						response = self.session.get(url2, headers=consts.sf_pc_detail_headers)
						text = utils.clean(utils.list_to_str(utils.clean_tags(response.text)))
						if '\\u' in text:
							text = text.encode('utf-8').decode('unicode_escape')
						item['intro'] += text
					...
					if isinstance(url3, str):
						if 'http' not in url3:
							url3 = 'https:' + url3
						response = self.session.get(url3, headers=consts.sf_pc_detail_headers)
					...
					logger.info(f'{item.url}获取详情页时出错:{e}')
					self.err_times += 1
			...
			logger.info(f'放弃获取详情页:{{item.url}}')
			self.err_times += 1
			return 0, item

	def expand_items(self, items:list):
		change_account_level = int(global_config.get('config','change_account_level'))
		disable_account_levle = int(global_config.get('config','disable_account_levle'))
		...
		while len(items) > 0:
			...
			if self.cookie_obj['err_times'] >= disable_account_levle:
			...
			if self.err_times <= 4:
				...
				self.err_times = 0
				logger.info(f'ali-sf的{item.url}最终放弃请求')
				continue

class Tasks(Err_Retry):
	def __init__(self):
		super().__init__()
	...
	def _run_(self):
		...
		for province in consts.sf_province:
			...
			repeat_set = db.get_items_repeat('ali', province[1])
