...
DB_CONNECT_STR = "mysql+pymysql://root:duanshihua133@localhost:3306/items?charset=utf8"
...
class Items(Base):
    ...
    id = Column(BigInteger,index=True,nullable=False,unique=True,comment="项目序列号id")
    url = Column(String(255),nullable=True,comment="项目拍卖链接")
    title = Column(String(255),nullable=False,index=True,comment="项目标题")
    phase = Column(Enum('一拍','二拍','重新拍卖','变卖','其他'),nullable=False,comment="拍卖阶段:一拍、二拍、重新拍卖、变卖、其他")

    category = Column(String(32),nullable=False,comment="项目种类,参考category表的值")

    repeat = Column(String(255),primary_key=True,comment="项目去重标志字段")
    ...
    margin = Column(String(255),nullable=True,comment="保证金")
    markup = Column(String(255),nullable=True,comment="加价幅度")
    ...
    updated_time = Column(TIMESTAMP,nullable=True,index=True,default=datetime.now,comment="更新时间")
    ...
    source = Column(String(255),primary_key=True,index=True,nullable=True,comment="项目来源网站")
    ...
    extra5 = Column(String(255),nullable=True,comment="预留位5,曾用来存储项目地图坐标")
...
class Category(Base):
    ...
    category = Column(String(255),nullable=False,comment="种类名")
    code = Column(String(255),nullable=True,comment="网站链接相关编码")
    extra1 = Column(String(255),nullable=True,comment="预留位1")
...
class Cookies(Base):
    ...
    useful = Column(Boolean,nullable=False,server_default=text('1'),comment="cookie是否仍可用")
    host = Column(String(255),nullable=False,index=True,comment="账号所属平台")
    extra = Column(String(255),nullable=True,comment="预留位1")
...
class Crawl_Log(Base):
	...
	err_level = Column(Integer,nullable=False,server_default=text('0'))
	extra = Column(String(255),nullable=True,comment="预留位1")
	occur_time = Column(TIMESTAMP,nullable=True,index=True,default=datetime.now,comment="发生时间")


def get_items_repeat(source, province_id=None):
    if province_id:
        all_id = db_session.query(Items.repeat).filter(and_(Items.source == source, Items.provinceId == province_id)).all()
    else:
        all_id = db_session.query(Items.repeat).filter(Items.source == source).all()
    return [i[0] for i in all_id]

def get_cookies(source, useful=True):
    return [{'cookie': cookie, 'err_times': 0} for cookie in db_session.query(Cookies).filter(and_(Cookies.host == source, Cookies.useful == useful)).all()]
